    ...
    <h3 class="history mb-4">Appointment Details</h3>
        <table class="table table-bordered">
            ...
            <div class="container text-center">
              <?php if($row['status']<3 && mysqli_num_rows($result)>0) {?>
                <button class="btn btn-primary " data-bs-toggle="modal" data-bs-target="#action">Cancel Appointment</button>
                <?php }
                else if($row['status']==4 && mysqli_num_rows($result)>0){?>
                <button class="btn btn-danger " >Appointment Cancelled</button>
                <?php } ?>
              </div>
          ...
          <div class="form-group">
              <label for="dates" required>Status:</label>
               <select class="category form-control" style="border-radius:20px;" name="status" placeholder="Select Catogery">
                      <option value="4">Cancel Appointment</option>
                </select>
          </div>
